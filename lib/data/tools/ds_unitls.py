...
from lib.data.tools.factory import xyxy2xywh, xywh2xyxy
from wbf.ensemble_boxes import *




help_url = 'https://github.com/ultralytics/rcnnv5/wiki/Train-Custom-Data'
...
def DataLoadModule(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False):
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            raise Exception('Error loading cfgs from %s. See %s' % (path, help_url))
        ...
        # Rectangular Training  https://github.com/ultralytics/rcnnv3/issues/232
        if self.rect:
...
def letterbox(img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
    # Resize image to a 32-pixel-multiple rectangle https://github.com/ultralytics/rcnnv3/issues/232
...
def reduce_img_size(path='../cfgs/sm4/images', img_size=1024):  # from utils.datasets import *; reduce_img_size()
...
def convert_images2bmp():  # from utils.datasets import *; convert_images2bmp()
    ...
    for path in ['../cfgs/sm4/images', '../cfgs/sm4/background']:
    ...
    for file in ['../cfgs/sm4/out_train.txt', '../cfgs/sm4/out_test.txt']:
        with open(file, 'r') as f:
            lines = f.read()
...
def recursive_dataset2bmp(dataset='../cfgs/sm4_bmp'):  # from utils.datasets import *; recursive_dataset2bmp()
...
def imagelist2folder(path='cfgs/coco_64img.txt'):  # from utils.datasets import *; imagelist2folder()

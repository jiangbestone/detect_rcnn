...
from lib.data.tools import torch_utils
...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    # Check anchor fit to cfgs, recompute if necessary
...
def check_anchor_order(m):
    # Check anchor order against stride order for RCNNv5 Detect() module m, and correct if necessary
...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    pr_score = 0.1  # score to evaluate P and R https://github.com/ultralytics/rcnnv3/issues/898
...
def kmean_anchors(path='./cfgs/dataset.yml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    if isinstance(path, str):  # *.yaml file
        ...
        from lib.data.tools.ds_unitls import LoadImagesAndLabels
...
def plot_wh_methods():
    ...
    plt.plot(x, ya, '.-', label='rcnn method')
...
def plot_study_txt(f='study.txt', x=None):
    ...
    for f in ['coco_study/study_coco_rcnnv5%s.txt' % x for x in ['s', 'm', 'l', 'x']]:
        ...
        ax2.plot(y[6, :j], y[3, :j] * 1E2, '.-', linewidth=2, markersize=8,
                 label=Path(f).stem.replace('study_coco_', '').replace('rcnn', 'RCNN'))
...
def plot_results(start=0, stop=0, bucket='', id=(), labels=()):
    # Plot training 'results*.txt' as seen in https://github.com/ultralytics/rcnnv5#reproduce-our-training
